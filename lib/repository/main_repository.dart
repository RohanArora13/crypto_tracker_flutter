...
class MainRepository {
  ...
  Future<DataModel> getCoinsList() async {
    DataModel data = DataModel(error: "something went wrong", data: null);
    try {
      print('$host/coins?skip=0&limit=15');
      var res = await _client.get(
        Uri.parse('$host/coins?skip=0&limit=15'),
        // headers: {
        //   'Content-Type': 'application/json; charset=UTF-8',
        // },
      );
      ...
      switch (res.statusCode) {
        case 200:
          CoinsList coinsData = CoinsList.fromJson(res.body);
          print(res.body);
          data = DataModel(
            error: null,
            data: coinsData,
